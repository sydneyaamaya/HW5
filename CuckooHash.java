 	...
 	public void put(K key, V value) {
        ...
        for (int i = 0; i <= CAPACITY; i++){
            ...
            //if it's not null
                ...
                pos2 = hash2(placed.getBucKey());
				//if the indexes of the placed pair's hash1 and the replaced pair's
				//hash2 are the same then the replaced pair is at it's 2nd index
				if (pos1 == pos2){
					pos2 = hash1(placed.getBucKey());
				}
                if (table[pos2] == null){
                    table[pos2] = placed;
                    return;
